...
import { CalendarDays, Plus, Trash2, CheckCircle, XCircle, CalendarOff, Coins, Sparkles } from 'lucide-react'
...
const REQ_STATUS: Record<string, string> = {
  pending_manager: 'bg-warning-soft text-warning-deep',
  pending_hr: 'bg-link-soft text-link-deep',
  approved: 'bg-success-soft text-success',
  rejected: 'bg-error-soft text-error-deep',
  cancelled: 'bg-canvas-soft-2 text-mute',
}
...
export default function LeavesPage() {
  ...
  const reqCols: any[] = [
    ...
    { key: 'dates', header: 'Dates', render: (r: LeaveRequestRow) => <span className="text-[12px] text-body">{dateShort(r.start_date)} → {dateShort(r.end_date)}</span> },
    { key: 'days', header: 'Days', render: (r: LeaveRequestRow) => <span className="text-[12px] font-medium text-ink">{r.days}</span> },
    ...
    { key: 'status', header: 'Status', render: (r: LeaveRequestRow) => <Badge className={REQ_STATUS[r.status]}>{reqLabel(r.status)}</Badge> },
    { key: 'actions', header: '', render: (r: LeaveRequestRow) => (
      <div className="flex items-center gap-1 justify-end flex-wrap">
        {canApproveManager && r.status === 'pending_manager' && <>
          <button onClick={() => { setApproveFor({ row: r, level: 'manager', action: 'approve' }); setRemarks('') }} className="px-1.5 py-0.5 text-[11px] text-success hover:bg-success-soft rounded-xs">Mgr Approve</button>
          <button onClick={() => { setApproveFor({ row: r, level: 'manager', action: 'reject' }); setRemarks('') }} className="px-1.5 py-0.5 text-[11px] text-error hover:bg-error-soft rounded-xs">Reject</button>
        </>}
        {isHr && r.status === 'pending_hr' && <>
          <button onClick={() => { setApproveFor({ row: r, level: 'hr', action: 'approve' }); setRemarks('') }} className="px-1.5 py-0.5 text-[11px] text-success hover:bg-success-soft rounded-xs font-medium">HR Approve</button>
          <button onClick={() => { setApproveFor({ row: r, level: 'hr', action: 'reject' }); setRemarks('') }} className="px-1.5 py-0.5 text-[11px] text-error hover:bg-error-soft rounded-xs">Reject</button>
        </>}
        {r.status.startsWith('pending') && <button onClick={() => cancelMut.mutate(r.id)} className="px-1.5 py-0.5 text-[11px] text-body hover:bg-canvas-soft rounded-xs">Cancel</button>}
      </div>
    ) },
  ]
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Applications, two-level approvals, balances, holidays and encashment"
        actions={tab === 'requests' ? <Button onClick={() => setShowApply(true)}><Plus className="w-3.5 h-3.5" /> Apply Leave</Button>
          : tab === 'balances' ? (
      ...
      {tab === 'requests' && (
        <div className="bg-white card-shadow rounded-md p-4">
          {isLoading ? <LoadingState /> : error ? <PageError onRetry={() => refetch()} /> : requests.length === 0 ? (
          ...
          ) : <Table columns={reqCols} data={requests} keyFn={(r) => String(r.id)} />}
        </div>
      )}

      {tab === 'balances' && (
        <div className="bg-white card-shadow rounded-md p-4">
          {balLoading ? <LoadingState /> : balError ? <PageError onRetry={() => balRefetch()} /> : balances.length === 0 ? (
            <EmptyState icon={Coins} title="No active employees" />
          ) : (
            <Table
              columns={[
                { key: 'emp', header: 'Employee', render: (r: any) => (
                  <div>
                    <p className="text-[13px] font-medium text-ink">{fullName(r.employee.first_name, r.employee.last_name)}</p>
                    <p className="text-[11px] text-mute">{r.employee.employee_code}</p>
                  </div>
                ) },
                ...types.filter((t: any) => t.paid_default === 1).map((t: any) => ({
                  key: `t${t.id}`, header: `${t.code}${t.accrual_monthly ? ' ⟳' : ''}`, render: (r: any) => {
                    ...
                    return <span className={`text-[12px] ${b.available < 0 ? 'text-error' : 'text-body'}`} title={`${b.accrued} accrued · ${b.used} used · ${b.pending} pending · ${b.comp_off_extra} comp-off · ${b.encashed} encashed`}>{b.available}</span>
                  },
                })),
                { key: 'actions', header: '', render: (r: any) => (
                  <div className="flex justify-end">
                    <button onClick={() => setBalFor(r.employee)} className="px-1.5 py-0.5 text-[11px] text-link hover:bg-link-soft rounded-xs">Details</button>
                  </div>
                ) },
              ]}
              data={balances}
          ...
          <p className="text-[11px] text-mute mt-3">⟳ = monthly accrual. Column value is available balance (accrued + comp-off − encashed − used − pending).</p>
        </div>
      )}

      {tab === 'holidays' && (
        <div className="bg-white card-shadow rounded-md p-4 max-w-xl">
          {holLoading ? <LoadingState /> : holError ? <PageError onRetry={() => holRefetch()} /> : (
            <>
              {isHr && (
                <div className="flex gap-2 mb-4">
                  <Input label="" type="date" value={holidayForm.date} onChange={e => setHolidayForm(f => ({ ...f, date: e.target.value }))} className="w-40" />
              ...
              {holidays.length === 0 ? <EmptyState icon={CalendarOff} title="No holidays configured" description="Holidays are excluded when counting leave days." /> : (
                <table className="w-full text-[13px]">
                  <tbody>
                    {holidays.map((h: any) => (
                      <tr key={h.id} className="border-b border-hairline last:border-0">
                        <td className="py-2 font-medium text-ink w-32">{dateShort(h.date)}</td>
                        <td className="py-2 text-body">{h.name}</td>
                        {isHr && <td className="py-2 text-right"><button onClick={() => setDelHolidayId(h.id)} className="px-1.5 py-0.5 text-[11px] text-error hover:bg-error-soft rounded-xs"><Trash2 className="w-3 h-3" /></button></td>}
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      <Modal open={showApply} onClose={() => setShowApply(false)} title="Apply Leave" size="sm">
        <div className="space-y-3">
          <select value={applyForm.employee_id} onChange={e => setApplyForm((f: any) => ({ ...f, employee_id: e.target.value }))} className="w-full h-9 px-2 text-[13px] bg-white border border-hairline rounded-sm outline-none focus:border-ink">
            <option value="">Select Employee</option>
            {employees.map(e => <option key={e.id} value={e.id}>{e.employee_code} — {fullName(e.first_name, e.last_name)}</option>)}
          </select>
          <select value={applyForm.leave_type_id} onChange={e => setApplyForm((f: any) => ({ ...f, leave_type_id: e.target.value }))} className="w-full h-9 px-2 text-[13px] bg-white border border-hairline rounded-sm outline-none focus:border-ink">
            <option value="">Leave Without Pay (no type)</option>
            {types.map(t => <option key={t.id} value={t.id}>{t.name} ({t.code}){Number(t.annual_quota) > 0 ? '' : ' — quota not set'}</option>)}
          </select>
          ...
          <p className="text-[11px] text-mute">Sundays and configured holidays are excluded from the day count.</p>
      ...
      <Modal open={!!balFor} onClose={() => setBalFor(null)} title={`Balances ${year} — ${balFor ? fullName(balFor.first_name, balFor.last_name) : ''}`} size="md">
        <div className="max-h-[55vh] overflow-y-auto">
          {(balDetail?.data || []).length === 0 ? <LoadingState /> : (
            <table className="w-full text-[12px]">
              <thead>
                <tr className="text-left text-mute border-b border-hairline">
                  <th className="py-1.5 font-medium">Type</th>
                  <th className="py-1.5 font-medium text-right">Entitled</th>
                  <th className="py-1.5 font-medium text-right">Accrued</th>
                  <th className="py-1.5 font-medium text-right">Used</th>
                  <th className="py-1.5 font-medium text-right">Pending</th>
                  <th className="py-1.5 font-medium text-right">Comp-off</th>
              ...
              <tbody>
                {((balDetail?.data || []) as BalanceRow[]).map(b => (
                  <tr key={b.leave_type_id} className="border-b border-hairline last:border-0">
                    <td className="py-1.5 text-ink font-medium">{b.name}{b.accrual_monthly ? ' ⟳' : ''}{b.is_comp_off ? ' ⭐' : ''}</td>
                    <td className="py-1.5 text-right text-body">{b.entitled}</td>
                    <td className="py-1.5 text-right text-body">{b.accrued}</td>
                    <td className="py-1.5 text-right text-body">{b.used}</td>
                    <td className="py-1.5 text-right text-body">{b.pending}</td>
                    <td className="py-1.5 text-right text-body">{b.comp_off_extra}</td>
                    <td className={`py-1.5 text-right font-medium ${b.available < 0 ? 'text-error' : 'text-ink'}`}>{b.available}</td>
                    <td className="py-1.5 text-right whitespace-nowrap">
                      {isHr && <>
                        <button onClick={() => { setCompoffForm({ leave_type_id: String(b.leave_type_id), days: '1', remarks: '' }) }} title="Credit 1 comp-off day (use Details form for more)" className="px-1 py-0.5 text-[11px] text-link hover:bg-link-soft rounded-xs"><Sparkles className="w-3 h-3 inline mr-0.5" />+1 Comp-off</button>
                        {b.available > 0 && b.paid && <button onClick={() => setEncashFor({ employee: balFor, row: b })} title="Encash available balance" className="px-1 py-0.5 text-[11px] text-body hover:bg-canvas-soft rounded-xs"><Coins className="w-3 h-3 inline mr-0.5" />Encash</button>}
                      </>}
                    </td>
                  </tr>
                ))}
        ...
        {isHr && (
          <div className="border-t border-hairline mt-3 pt-3 flex items-end gap-2">
            <select value={compoffForm.leave_type_id} onChange={e => setCompoffForm(f => ({ ...f, leave_type_id: e.target.value }))} className="h-9 px-2 text-[13px] bg-white border border-hairline rounded-sm outline-none focus:border-ink flex-1">
              <option value="">Select type for adjustment…</option>
              {types.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
            </select>
            <Input label="" type="number" placeholder="±days" value={compoffForm.days} onChange={e => setCompoffForm(f => ({ ...f, days: e.target.value }))} className="w-24" />
